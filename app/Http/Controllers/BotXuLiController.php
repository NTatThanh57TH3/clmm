...
class BotXuLiController extends Controller
{
    ...
    public function send($method,$parameters){
		$bot_token='1993282631:AAHO4fVI2xLukFvbnOJ7r_C2FxzUUrjxiGA';
                            ...
                            if ( $res['comment'] == 'h1' ) {
                                ...
                                if ($Check == 0) {
                                    ...
                                    if ($NameGame != '') {
                	                    ...
                	                    $ketquagane= $LichSuChoiMomo->ketqua == '1' ? ' *THẮNG* ' : ' ~~THUA~~ ' ;
                ...
                if ($Check->tiennhan > 0) {
                        ...
                        if($Check->status == 3){
                            $parameters = array(
                        	             ...
                        	             $parameters["text"]='*Trả Thưởng SDT* ' .    $Check->sdt  .' Game '.  $Check->trochoi .'  Mã Giao Dịch : ' . $Check->magiaodich . ' Nội Dung : ' .  $Check->noidung  .' Cược: '. number_format($Check->tiencuoc)  .'  Tiền nhận ' . number_format($Check->tiennhan) . '  *THÀNH CÔNG* ' ;
                        	            $this->send("sendMessage",$parameters);
                        }
                ...
                if ($Check->tiennhan > 0) {
                        ...
                        if ( isset($res['status']) && $res['status'] == 200) {
                             ...
                             $parameters = array(
                	            ...
                	            $parameters["text"]='*Xử lý giao dịch lỗi*: SDT ' . $Check->sdt .' Game '. $Check->trochoi .' Mã giao dịch: ' .$Check->magiaodich  . ' Nội dung: ' . $Check->noidung  .' Cược: '. number_format($Check->tiencuoc)  .' Tiền nhận ' . number_format($Check->tiennhan);
                	            $this->send("sendMessage",$parameters);
